 ...
 * |> Since: 4/3/16
 */
public class GlassTheme implements GUITheme {
    ...
    public void drawCategories(List<Node> categories, Node node, float mouseX, float mouseY) {
        ...
        if (node.isExtended) {
            GuiUtil.drawSexyRect(top.x, top.y, bottom.width, bottom.height, 0x50232323, 0x90232323);
        } else {
            GuiUtil.drawSexyRect(top.x, top.y, top.width, top.height, 0x50232323, 0x90232323);
        }
    ...
    public void drawModules(List<Node> categories, Node node, float mouseX, float mouseY) {
        ...
        for (SubNode<Module> snode : (List<SubNode<Module>>) node.subset) {
            ...
            if (Methods.inBounds(mouseX, mouseY, button) && !Client.getInstance().getGui().isAnyNodeBeingDragged()) {
                GuiUtil.drawRect(button.x, button.y, button.width, button.height, 0x601c1c1c);
            }
            font.drawString(module.getName(), node.x + 3F, y - 1, NahrFont.FontType.SHADOW_THICK, module.isRunning() ? 0xFFfcffd0 : 0xFFEEEEEE, 0x20171717);
            if (Methods.inBounds(mouseX, mouseY, button) && !Client.getInstance().getGui().isAnyNodeBeingDragged())
